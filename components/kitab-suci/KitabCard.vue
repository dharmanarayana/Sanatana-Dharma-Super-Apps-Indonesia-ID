  ...
  <div class="card overflow-hidden group cursor-pointer hover:border-brand/40 transition-all flex flex-col h-full">
    <div class="aspect-[3/4] bg-surface-variant relative overflow-hidden shrink-0">
      ...
      <div class="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
        ...
        <p class="text-white font-black italic text-sm leading-tight group-hover:text-brand transition-colors">
          {{ kitab.title }}
        </p>
      </div>
    </div>
    <div class="p-3 mt-auto bg-surface">
      <div class="flex items-center justify-between text-[10px] text-muted font-bold">
        <span class="flex items-center gap-1">
          <Icon name="lucide:layout-list" size="10" />
          DETAIL KITAB
        </span>
        <Icon name="lucide:chevron-right" size="12" class="group-hover:translate-x-1 transition-transform" />
      </div>
    </div>
  </div>
</template>
