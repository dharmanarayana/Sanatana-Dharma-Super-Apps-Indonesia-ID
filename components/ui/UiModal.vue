    ...
    <Teleport to="body">
      <Transition :name="isDesktop ? 'modal-center' : 'modal-bottom'">
        <div v-if="show"
          ...
          <!-- Panel modal -->
          <div class="relative z-10 bg-[var(--bg-surface)] w-full"
               :class="[
                 isDesktop
            ...
            <!-- Handle bar mobile -->
            <div v-if="!isDesktop"
                 class="w-10 h-1 bg-[var(--border-strong)] rounded-full mx-auto mt-3 mb-1" />

            <!-- Header modal -->
            <div v-if="title || $slots.header"
                 class="flex items-center justify-between px-5 py-4
                        border-b border-[var(--border-default)]">
              <h3 class="font-bold text-[var(--text-primary)] text-lg">{{ title }}</h3>
              <button @click="$emit('close')"
                      class="touch-target rounded-full text-[var(--text-muted)] hover:text-[var(--text-primary)]"
                      aria-label="Tutup">
                ✕
              </button>
            </div>
